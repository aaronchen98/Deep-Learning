...
def compute_loss(predictions, targets, anchors, scales, num_classes=20, output_size=(13, 13)):
    ...
    # class probability [-1, H*W, B, C] ，类别概率——one hot形式，C维
    _probs = targets["probs"]
    # 1 for object, 0 for background, [-1, H*W, B]，置信度，每个边界框一个
    _confs = targets["confs"]
    ...
    coords_wh = tf.sqrt(
        tf.exp(coords[:, :, :, 2:4])*anchors/np.reshape([W, H], [1, 1, 1, 2]))
    # [batch_size, H*W, B, 4]
