...
def conv2d(x, filters_num, filters_size, pad_size=0, stride=1, batch_normalize=True,
    ...
    if pad_size > 0:
        x = tf.pad(x, [[0, 0], [pad_size, pad_size],
    ...
    if batch_normalize:
        out = tf.layers.batch_normalization(
            out, axis=-1, momentum=0.9, training=False, name=name+'_bn')
    if activation:
        out = activation(out)
    return out
...
def darknet(images, n_last_channels=425):
    net = conv2d(images, filters_num=32, filters_size=3,
                 pad_size=1, name='conv1')
